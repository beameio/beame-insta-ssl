...
class writeToTunnel{
	...
	initWriter(data){
		...
		while(count > 0){
			count -= 1;
			if(count < 1 && (lastChunk && lastChunk.length > 0)){
				dataChunk = data.slice(step*chunkSize, lastChunk);
				this._fifoOut.push(this.adjustOutputBuffer(count, dataChunk));
			}
			else{
				dataChunk = data.slice(step*chunkSize, chunkSize);
				this._fifoOut.push(this.adjustOutputBuffer(count, dataChunk));
			}
			step += 1;
		}
...
class bufferedWriter {
	...
	writeData(data, socket){
		...
		if(!this._socket)
			this._socket = socket;
		console.log(this._socket.id,' writing data:', data.byteLength);
...
class bufferedReader{
	...
	buildFrame(data){
		...
		let chunkNumber = Number(data.slice(uuidLength, bufferDepth).toString());
		if(chunkNumber != (this._packetCount - 1)){
			console.log('Proebali packet:', chunkNumber,'!=',this._packetCount);
		}
		this._packetCount = chunkNumber;
		let rawData = data.slice(headerLength, data.byteLength);
		...
		console.log(id,'=>got data:', data.byteLength, ' id:',id, ' out:',output.byteLength,' ',output);

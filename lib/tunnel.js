	...
	return new Promise((resolve, reject) => {
		...
		try {
			...
			io.on('connection', (socket) =>{
				...
				if(highAncestor){
					store.verifyAncestry(cred.fqdn, peerData.subject.CN, highAncestor, lowCredLevel, (error, related) => {
						if(!related) {
							console.warn('Failed to verify certificate <', peerData.subject.CN, '> for: ', cred.fqdn);
							socket.emit('_end_','Client certificate validation failed');
							socket.disconnect();
						}
						else onClientAllowed(peerData.subject.CN);
					});
				}
				else onClientAllowed(peerData.subject.CN);

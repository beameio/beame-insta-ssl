	...
	if(id && !localSockets[id]){
		...
		try{
			...
			localSockets[id].on('data', (data)=>{
				...
				terminatingSockets[id] && terminatingSockets[id].emit('data', Buffer.from(rawData));
			...
			localSockets[id].on('connect', () => {
				...
				if (localBuffer.length > 0) {
					if(localSockets[id].write(Buffer.from(localBuffer))){
						localBuffer = [];
					}
				}
			});
		...
		try {
			...
			io.on('connection', (socket) =>{
				...
				if(highAncestor && hasAuth){
					...
					let options = {
						highestFqdn: highAncestor,
						trustDepth : lowCredLevel
					};

					store.verifyAncestry(cred.fqdn, cn, options, (error, related) => {
						if(!related) {

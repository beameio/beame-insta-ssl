...
function tunnel(cred, targetHost, targetPort, targetProto, targetHostName, highestFqdn, trustDepth) {
	...
	switch(targetProto) {
		case 'tcp':
			...
			startTCPproxy(cred, targetHostName, (port) => {
				if(!proxyClient && port){
...
function startTCPproxy(cred, targetHostName, cb) {

	startTerminatingHttpsServer(cred, targetHostName).then((port)=>{
		cb(port);
	}).catch(e=>{
		console.error(e);
		cb(0);
...
const _startDstClient = (id, targetHostName) => {
	...
	if(id && !localSockets[id]){
		console.log('_startDstClient =>', id);
...
function startTerminatingHttpsServer(cred, targetHostName) {
	let httpsServer;
				...
				const initSocket = (socket) => {
					...
					socket.on('command', (data)=>{
						...
						if(cmd === utils.connectedStr){
							console.warn(id,' => Client app connected');
							localBuffer = [];
							socket.id = id;

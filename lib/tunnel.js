	...
	switch(targetProto) {
		case 'tcp':
			...
			startTCPproxy(cred, 55333, (flag) => {
				if(!proxyClient && flag){
					proxyClient = new ProxyClient("HTTPS", cred, 'localhost', 55333, {}, null, serverCerts, true);
...
function startTCPproxy(cred, targetPort, cb) {
	...
	startTerminatingHttpsServer(cred, targetPort).then(()=>{
		cb(true);
	}).catch(e=>{
		console.error(e);
		cb(false);
...
function startTerminatingHttpsServer(cred, targetPort) {
	console.log('starting TerminatingHttpsServer on:', targetPort);
	return new Promise((resolve, reject) => {
		...
		try {
			...
			io.on('connection', (socket) =>{//{certs, requestCert: false}, (socket) =>{
				const onData = (rawData, id) => {
					if(!localSockets[id]){
						localBuffer.push.apply(localBuffer,rawData);//seems irrelevant
					}
				...
				setId(()=>{
					...
					socket.on('drain',()=>{
						console.log('terminatingSocket <', socket.id,'> drain');
						// bufferedWriter[socket.id] && bufferedWriter[socket.id].drainData(socket);
						socket.resume();
					})
					socket.on('close',(had_error)=>{
						console.log('terminatingSocket',socket.id,'close:', had_error);
